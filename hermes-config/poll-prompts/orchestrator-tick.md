...
Repo to operate on: read from environment variable `AGENCY_REPO` (format: `<owner>/<name>`). If unset, exit immediately with `NO_REPO`.
...
2. **Pull latest state.** `cd` into the local clone of `$AGENCY_REPO` (path: `~/.hermes-cache/$AGENCY_REPO`). If it does not exist, clone it. Run `git pull --rebase`.

3. **Regenerate state.** Run `pwsh scripts/rebuild-state.ps1 -Repo $AGENCY_REPO`. Commit `State.md` if it changed.

4. **Promote ready tasks.** Run:
   ```
   gh issue list --repo $AGENCY_REPO --label "backlog" --state open --json number,labels,body --limit 100
   ```
...
5. **Dispatch ready tasks.** Run:
   ```
   gh issue list --repo $AGENCY_REPO --label "ready" --state open --json number,title,labels,assignees --limit 50
   ```
...
6. **Check progress.** For each `in-progress` issue, check the last commit time on its branch via `gh api repos/$AGENCY_REPO/branches/<branch>`. If older than 30 minutes, post a warning comment. If older than 60 minutes, add label `blocked` and escalate (see step 9).
